...
public class TrgovinaController {
    ...
    public ResponseEntity<?> kreirajTrgovinu(@RequestPart("dto") TrgovinaDTO dto, @RequestPart(value = "logo") MultipartFile logoFile, @RequestPart(value = "fotografije", required = false) List<MultipartFile> fotografije) throws IOException {
        try {
            Trgovina t = trgovinaService.kreirajTrgovinu(dto, logoFile);
    ...
    public ResponseEntity<?> ažurirajTrgovinu(@PathVariable Integer id, @RequestPart("dto") TrgovinaDTO dto, @RequestPart(value = "logo", required = false) MultipartFile logoFile, @RequestPart(value = "fotografije", required = false) List<MultipartFile> fotografije) throws IOException {
        try {
            Trgovina t = trgovinaService.azurirajTrgovinu(id, dto, logoFile);
            slikaTrgovineService.spremiSlike(fotografije, t);
            return ResponseEntity.status(HttpStatus.CREATED).body(Map.of(
